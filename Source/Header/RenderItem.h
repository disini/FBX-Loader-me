...
struct SkinnedModelInstance
{
	...
	void UpdateSkinnedAnimation(std::string ClipName, float dt)
	{
		...
		if (TimePos > SkinnedInfo->GetClipEndTime(ClipName))
		{
			...
			if (ClipName == "LittleDragon" || ClipName == "Walking")
			{
				TimePos = 0.0f;
			}
		}
		...
		//if (ClipName == "Idle")
		if (ClipName == "LittleDragon")
		{
			state = eClipList::Idle;
		}
		else if (ClipName == "StartWalking")
			state = eClipList::StartWalking;
		else if (ClipName == "Walking" || ClipName == "WalkingBackward1" || ClipName == "run" || ClipName == "playerWalking")
