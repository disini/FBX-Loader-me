...
void PortfolioGameApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	else if (GetAsyncKeyState('S') & 0x8000)
	{
		if (!mCameraDetach)
		{
			mPlayer.SetClipName("WalkingBackward1");
	...
	else
	{
		if (mPlayer.isClipEnd())
			...
			mPlayer.SetClipName("LittleDragon");
	}
...
void PortfolioGameApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.AmbientLight = { 0.85f, 0.85f, 0.85f, 1.0f };
...
void PortfolioGameApp::LoadTextures()
{
	...
	mTexDiffuse.SetTextures(texNames, texPaths);
...
void PortfolioGameApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&boxRitem->World, XMMatrixTranslation(0.0f, 0.0f, 0.0f) * XMMatrixScaling(1.0f, 4.0f, 1.0f));
