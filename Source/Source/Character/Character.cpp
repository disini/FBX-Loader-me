	...
	std::string geoName)
{
	...
	const int numOfSubmesh = vSubmeshOffset.size();
	//const int numOfSubmesh = (const int)((std::vector<int>)vSubmeshOffset.size());
	...
	for (int i = 0; i < numOfSubmesh; ++i)
	{
		...
		UINT CurrSubmeshOffsetIndex = vSubmeshOffset[i];// 9, 2151, 2775, .......

		SubmeshGeometry FbxSubmesh;
		FbxSubmesh.IndexCount = CurrSubmeshOffsetIndex;//9, 2151, 
		FbxSubmesh.StartIndexLocation = SubmeshOffsetIndex;// 0, 9, 
		FbxSubmesh.BaseVertexLocation = 0;

		std::string SubmeshName = vBoneName[i];// "Hips","Spine"......
		geo->DrawArgs[SubmeshName] = FbxSubmesh;

		SubmeshOffsetIndex += CurrSubmeshOffsetIndex;// 9
	}
