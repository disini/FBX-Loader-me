	...
	Textures& mTextures, Textures& mTexturesNormal, Materials& mMaterials)
{
	...
	for (int i = 0; i < outMaterial.size(); ++i)
	{
		...
		if (!outMaterial[i].Name.empty())
		{
			...
			//if (outMaterial[i].Name == "D:\\Dev\\Developing\\DirectX\\CharacterTool\\Resource\\FBX\\Character\\Idle.fbm/Kachujin_diffuse.png")
			if (outMaterial[i].Name.find("D:") != outMaterial[i].Name.npos)
			{
				printf("Absolute Path found!");
				outMaterial[i].Name = "../../Resource/FBX/Character/LittleDragon.fbm/ToonDragon_diffuse.jpg";
			}
			TextureFileName.assign(outMaterial[i].Name.begin(), outMaterial[i].Name.end());
			mTextures.SetTexture(
				TextureName,// "playerTex0"
				TextureFileName);// L"../../Resource/FBX/Character/Idle.fbm/Kachujin_diffuse.png"

			// Normal Map
			std::wstring TextureNormalFileName;
			TextureNormalFileName = TextureFileName.substr(0, TextureFileName.size() - 11);
			TextureNormalFileName.append(L"normal.jpg");// L"../../Resource/FBX/Character/Idle.fbm/Kachujin_normal.jpg"
			struct stat buffer;
...
void FBXGenerator::LoadFBXPlayer(Player& mPlayer, Textures& mTextures, Textures& mTexturesNormal, Materials& mMaterials)
{
	...
	fbx.LoadFBX(outSkinnedVertices, outIndices, outSkinnedInfo, "LittleDragon", outMaterial, FileName);
	...
	fbx.LoadFBX(outSkinnedInfo, "WalkingBackward1", FileName);*/
	//fbx.LoadFBX(outSkinnedInfo, "WalkingBackward2", FileName);
