...
{
	// if exported animation exist
	if (LoadAnimation(outSkinnedData, clipName, fileName)) 
		return S_OK;
	...
	std::string fileName)
{
	...
	if (fileIn)
	{
		...
		for (uint32_t i = 0; i < boneSize; ++i)
		{
			...
			//printf("FbxLoader::LoadSkeleton() : bone index == , %d, bone name == %s \n;", i, tempBoneName);

			std::wstring text =
			L"FbxLoader::LoadSkeleton() : bone index == " + std::to_wstring(i) + 
			L", bone name == " + s2ws(tempBoneName) +
			L"\n";
		...
		for (uint32_t i = 0; i < boneSize; ++i)
		{
			...
			//printf("FbxLoader::LoadSkeleton() : bone offset index == , %d, bone name == %s \n;", i *4 + j, tempBoneOffset);
		...
		for (uint32_t i = 0; i < boneSize; ++i)
		{
			...
			outSkinnedData.SetSubmeshOffset(tempBoneSubmeshOffset);//9, 
			//printf("FbxLoader::LoadSkeleton() : bone submesh offset ==  %d \n;", i, tempBoneSubmeshOffset);
			std::wstring text =
				L"FbxLoader::LoadSkeleton() : bone submesh offset == " + std::to_wstring(tempBoneSubmeshOffset) +
				L"\n";
	...
	std::vector<Material>* outMaterial)
{
	...
	std::string fullFileName = fileName + ".cmesh";
	std::ifstream fileIn(fullFileName);
	...
	if (fileIn)
	{
		...
		if (outMaterial != nullptr)
		{
			...
			for (uint32_t i = 0; i < materialSize; ++i)
			{
				...
				fileIn >> ignore >> tempMaterial.Name;// 正确的相对路径或写死的完整绝对路径
				tempMaterial.Name = fileName + ".fbm/" + tempMaterial.Name;// tempMaterial.Name只是文件名；（我加的）
				fileIn >> ignore >> tempMaterial.Ambient.x >> tempMaterial.Ambient.y >> tempMaterial.Ambient.z;
				fileIn >> ignore >> tempMaterial.DiffuseAlbedo.x >> tempMaterial.DiffuseAlbedo.y >> tempMaterial.DiffuseAlbedo.z >> tempMaterial.DiffuseAlbedo.w;
				fileIn >> ignore >> tempMaterial.FresnelR0.x >> tempMaterial.FresnelR0.y >> tempMaterial.FresnelR0.z;
	...
	int curIndex, int parentIndex)
{
	...
	int pNodeChildCount = pNode->GetChildCount();
	//for (int i = 0; i < pNode->GetChildCount(); ++i)
	for (int i = 0; i < pNodeChildCount; ++i)
	{
